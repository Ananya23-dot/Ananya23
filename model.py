    ...
    def detect_patterns(self, img, disease_patterns):
        ...
        try:
            ...
            for pattern_name, pattern_info in disease_patterns.items():
                ...
                mask = cv2.inRange(
                    hsv, pattern_info['hsv_lower'], pattern_info['hsv_upper'])
        ...
        try:
            ...
            for disease_key, disease_info in self.diseases.items():
                ...
                pattern_scores = self.detect_patterns(
                    img, disease_info['patterns'])

                if pattern_scores:
                    # Calculate weighted average of pattern scores
                    total_weight = sum(
                        disease_info['patterns'][p]['weight'] for p in pattern_scores.keys())
                    disease_score = sum(
                        score for score in pattern_scores.values()) / total_weight
            ...
            if max_disease == 'healthy' and len(disease_scores) > 1:
                other_scores = {k: v for k,
                                v in disease_scores.items() if k != 'healthy'}
                if any(score > 0.2 for score in other_scores.values()):
                    max_disease = max(other_scores.items(),
                                      key=lambda x: x[1])[0]
